...
export type SaleSubmission = {
  ...
  extraCosts: { label: string; value: number }[];
  fiadoDueDate: string | null;
  fiadoDownPayment: number | null;
};
...
export default function SellProductModal({
  ...
  onClose: () => void;
  onConfirm: (submission: SaleSubmission) => Promise<boolean>;
}) {
  ...
  const baseCost = totalCost(product);
  ...
  const confirm = async () => {
    ...
    const ok = await onConfirm({
      ...
      tradeInDescription: tradeInEnabled ? tradeInDescription.trim() || null : null,
      tradeInValue: tradeInEnabled ? tradeInValueNum : null,
      extraCosts: extraCostsNum.filter((e) => e.label.length > 0 || e.value > 0),
      fiadoDueDate: method === "Fiado" ? fiadoDueDate || null : null,
      fiadoDownPayment: method === "Fiado" ? fiadoDownPaymentNum : null,
    });
    setSaving(false);
    if (!ok) {
  ...
  return (
    <div
      onClick={onClose}
      className="fixed inset-0 z-[65] flex items-start justify-center overflow-y-auto bg-black/55 p-4 py-8"
    >
      <div
